...
public:
    ...
    void solve(vector<Result> &results)
    {
        ...
        try
        {
            ...
            // cplex.setParam(IloCplex::Param::Emphasis::Memory, 1); // Conserve memory where possible
            if (timelimit->back() > 0)
            {
            ...
            // Log
            ofstream fout("bin/ct_world.log");
            cplex.setOut(fout);
...
int main(int argc, char *argv[])
{
    ...
    // Setup...
    MaxDiversityProblemLowMemory mdp(input_filepath, p_ratio, 3);
    ...
    output << "Solution: (included cities starting at 1):" << endl;
    for (int i = 0; i < mdp.p; i++)
    {
        output << ct.solution[i] << " ";
    }
    output << endl;
    output.close();
